...
public class TestPreparedData {
	...
	public void shouldVerifyStructureWithDepth1() {
		...
		for (int i = 0; i < workbaskets.size(); i++) {
			...
			String ecpectedOrgLvl1 = Formatter.format(i, WorkbasketWrapper.NUMBER_LENGTH_IN_ID);
	...
	public void shouldVerifyStructureWithDepth2() {
		...
		for (int i = 0; i < workbasketsLvl1.size(); i++) {
			...
			String expectedOrgLvl1 = Formatter.format(i, WorkbasketWrapper.NUMBER_LENGTH_IN_ID);
			...
			for (int j = 0; j < wbLvl1.getDirectChildren().size(); j++) {
				...
				assertThat("OrgLvl2", wbLvl2.getOrgLevel2(), equalTo(expectedOrgLvl2));
			}
		}
	}
		...
		for (int i = 0; i < workbasketsLvl1.size(); i++) {
			...
			for (int j = 0; j < wbLvl1.getDirectChildren().size(); j++) {
				...
				for (int k = 0; k < wbLvl2.getDirectChildren().size(); k++) {
					...
					String expectedOrgLvl3 = Formatter.format(wbLvl2.getMemberId(), WorkbasketWrapper.NUMBER_LENGTH_IN_ID);
	...
	public void shouldVerifyStructureWithDepth4() {
		...
		for (int i = 0; i < workbasketsLvl1.size(); i++) {
			...
			for (int j = 0; j < wbLvl1.getDirectChildren().size(); j++) {
				WorkbasketWrapper wbLvl2 =  wbLvl1.getDirectChildren().get(j);
				String expectedOrgLvlWbLvl2 = expectedOrgLvlWbLvl1 +  Formatter.format(wbLvl1.getMemberId(), WorkbasketWrapper.NUMBER_LENGTH_IN_ID);

				for (int k = 0; k < wbLvl2.getDirectChildren().size(); k++) {
					WorkbasketWrapper wbLvl3 = wbLvl2.getDirectChildren().get(k);
					String expectedOrgLvlWbLvl3 = expectedOrgLvlWbLvl2 + Formatter.format(wbLvl2.getMemberId(), WorkbasketWrapper.NUMBER_LENGTH_IN_ID);

					for (int l = 0; l < wbLvl3.getDirectChildren().size(); l++) {
						WorkbasketWrapper wbLvl4 =  wbLvl3.getDirectChildren().get(l);

						String expectedOrgLvl4 = Formatter.format(wbLvl3.getMemberId(), WorkbasketWrapper.NUMBER_LENGTH_IN_ID);
	...
	private String getExpectedOwner(WorkbasketWrapper parent, WorkbasketWrapper child, String compoundOrgLvl) {
		...
		if (parent != null && child.getOwner().equals(parent.getOwner())) {
			...
			expectedOwner = DOMAIN + role + compoundOrgLvl + UserWrapper.ID_PART_LINKING_CHAR
					+ Formatter.format(currentTeamIndex, UserWrapper.NUMBER_LENGTH_IN_ID);
		}
